  ...
  def config
    ...
    OptionParser.new do |opts|
      ...
      opts.on('-bs3_bucket', '--s3-bucket=s3_bucket',
              's3 bucket on aws for the output and artifacts') do |s3|
      ...
      opts.on('-uuser', '--user=user',
              'only do tests for commits by this github user') do |user|
        options[:user] = user
      end

      opts.on('-ffull_name', '--full-name=full_name',
              '***project full name, like "nroose/sqs_ci".') do |full_name|
        options[:full_name] = full_name
      end

      opts.on('-gcommit_ref', '--commit-ref=commit_ref',
              '***run tests on this commit ref, or ' \
              'branch name.') do |commit_ref|
      ...
      opts.on('-v', '--verbose', 'full output.') do
        options[:verbose]
      end
